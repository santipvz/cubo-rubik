    ...
    def heuristicaPosicionesCorrectas(self):
        ...
        for cara in self.caras:
            for casilla in cara.casillas:
                if casilla.posicionCorrecta == cara.color:  # Si la casilla está en su posición correcta
                    heuristica += 1
        return heuristica


    def heuristicaCasillasDescolocadas(self):
        # Inicializamos el contador de casillas incorrectas a 0
        casillas_incorrectas = 0

        # Iteramos sobre cada cara del cubo
        for cara in self.caras:
            # Iteramos sobre cada casilla en la cara actual
            for casilla in cara.casillas:
                # Si el color de la casilla no coincide con el color de la cara, incrementamos el contador
                if casilla.color != cara.color:
                    casillas_incorrectas += 1

        # Devolvemos el número total de casillas incorrectas
        return casillas_incorrectas
